    ...
    if($(".Selct").is(":checked")){
            var valida = 0
                 $("input:checkbox:checked").each(function() {
                    var cant = parseFloat($(this).attr("cant"));
                    var rest = parseFloat($(this).attr("rest"));
                    var idp = $(this).attr("preoc");
                    //alert(' Cant: ' + cant + ' Rest: ' + rest );
                    if(cant <= 0 || cant > rest){
                        valida = parseFloat(valida) + 1;
                        //alert('entro al if');
                          $.ajax({
                            url:"index.php",
                            method:"POST",
                            dataType:"json",
                            data:{actCanti:1, idpreoc:idp, rest:rest},
                            success:function(data){
                                if(data.resultado == 1){
                                    alert('Se actualizo el restante');
                                }
                            }
                        });
...
function validaCant(a,b,c,d, can){
    ...
    if(cantn > 0){
        ...
        alert('La cantidad sebe ser mayor a 0');
        document.getElementById("cantidad_" + c).value = cantm;
    }
