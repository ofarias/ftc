...
class pegaso_ventas extends database{
    ...
    ***/
    ...
    function articuloXrfc($cliente){
        ...
        $this->query="SELECT CLAVE FROM CLIE01 WHERE RFC = '$row->RFC'";
...
/*
        $this->query="SELECT cve_art as id, max(p.nombre) AS NOMBRE, max(p.categoria) AS CATEGORIA, max(p.precio) AS PRECIO, max(p.MARCA) AS MARCA, max(p.medidas) as medidas, max(p.COSTO_VENTAS) as costo,  0 as MARGEN_MINIMO, '' as MARGEN_BAJO
             ...
             left join FTC_COTIZACION_DETALLE d on d.CDFOLIO = c.CDFOLIO
             left join producto_ftc p on p.clave_ftc = d.cve_art
             where trim(c.cve_cliente) = trim('$key->CLAVE') and p.status = 'A' group by d.cve_art";
                $rs=$this->EjecutaQuerySimple();
  */      
        ...
        $data2=array();
        foreach ($data as $key) {
            $this->query="SELECT cve_art as id, max(p.nombre) AS NOMBRE, max(p.categoria) AS CATEGORIA, max(p.precio) AS PRECIO, max(p.MARCA) AS MARCA, max(p.medidas) as medidas, max(p.COSTO_VENTAS) as costo,  0 as MARGEN_MINIMO, '' as MARGEN_BAJO, max(INVE.CAMPLIB2) as sku,
             ...
             left join FTC_COTIZACION_DETALLE d on d.CDFOLIO = c.CDFOLIO
             left join producto_ftc p on p.clave_ftc = d.cve_art
             LEFT JOIN inve_clib01 inve on inve.cve_prod  = ('PGS'||d.cve_art)
             where trim(c.cve_cliente) = trim('$key->CLAVE') and p.status = 'A' group by d.cve_art";
                $rs=$this->EjecutaQuerySimple();
            //echo 'Consulta de productos por RFC del cliente '.$key->CLAVE.'  '.$this->query;
    ...
    function insertaCotizacion($cliente, $identificadorDocumento){
        ...
        $usuario = $_SESSION['user']->USER_LOGIN;
        $this->query = "INSERT INTO FTC_COTIZACION (CDFOLIO, CVE_CLIENTE, DSIDEDOC, DTFECREG, INSTATUS, DBIMPSUB, DBIMPIMP, DBIMPTOT, DSPLANTA, DSENTREG, CDUSUARI, FOLIO, SERIE, cve_cotizacion, vendedor ) "
                . "VALUES ($folio, TRIM('$cliente'), '$identificadorDocumento', CAST('Now' as date),'PENDIENTE',0,0,0,(SELECT COALESCE(substring(CAMPLIB7 from 1 for 90), '') FROM CLIE_CLIB01 WHERE TRIM(CVE_CLIE) = TRIM('$cliente')),(SELECT COALESCE(substring(CAMPLIB8 from 1 for 90), '') FROM CLIE_CLIB01 WHERE TRIM(CVE_CLIE) = TRIM('$cliente')),'$usuario', $foliol, '$letra', '$letra'||'$foliol', '$usuario')";        
